let interests: string[] = ['Investing', 'Learning Languages', 'Craftsmanship', 'Classic Racing Cars']; // interests
var prevcomp: string[] = ['Social Enterprise', 'Tech Start Up', 'Birmingham Southern - College']; // my previous company
var prevjob: string[] = ['CashFlow dude', 'Strategist and Brainstormer', 'Teaching Assistant']; // my previous job title
var prevdesc: string[] = ['VP of Finance', 'Co-Founder', 'Teaching Assistant for college students in Business and Finance classes']; // my previous description
var skills: string[] = ['Project Management', 'Number Cruncher', 'Online Sim Racer', 'Microsoft Office Suite', 'Violinist and Guitarist', 'Finance', 'Innovation Seeker', '2nd Voicing to Lead Vocals', 'Capital Outlays']; // my skills 

function displayName(nameObj: {
    name: string
}) {
    let name: string = nameObj.name;
    let displayName: string = name.toLowerCase().split(' ').map(name => name.charAt(0).toUpperCase() + name.substring(1)).join(' ');
